...
namespace PHARMACYMS.PharmacistUC
{
    public partial class UC_P_Dashboard : UserControl
    {
        ...
        public void loadChart()
        {
            ...
            this.chart1.Series["Valid Medicines"].Points.AddXY("Medicine Validity Chary", count);

            query = "select count(mname) from medicine where eDate <= getdate()";
            ds = fn.getData(query);
            count = Int64.Parse(ds.Tables[0].Rows[0][0].ToString());
            this.chart1.Series["Expired Medicines"].Points.AddXY("Medicine Validity Chary", count);
        }
